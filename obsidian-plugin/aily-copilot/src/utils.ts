...
export function isPlusChain(chainType: ChainType): boolean {
  return (
    chainType === ChainType.AILY_CHAIN ||
...
export async function checkLatestVersion(): Promise<{
  ...
  // Aily Copilot is a local fork of Obsidian Copilot. Do not prompt users to
  // install upstream Copilot releases over this fork.
...
export function getNeedSetKeyProvider(): Provider[] {
  return [ChatModelProviders.DEEPSEEK, ChatModelProviders.KIMI];
}
