...
const EMBEDDING_PROVIDER_CONSTRUCTORS = {
  [EmbeddingModelProviders.KIMI]: CustomOpenAIEmbeddings,
  [EmbeddingModelProviders.DEEPSEEK]: CustomOpenAIEmbeddings,
...
export default class EmbeddingManager {
  ...
  private readonly providerApiKeyMap: Record<EmbeddingModelProviders, () => string> = {
    [EmbeddingModelProviders.KIMI]: () => getSettings().kimiApiKey,
    [EmbeddingModelProviders.DEEPSEEK]: () => getSettings().deepseekApiKey,
  ...
  private async getEmbeddingConfig(customModel: CustomModel): Promise<Record<string, unknown>> {
    ...
    const providerConfig: {
      ...
      [EmbeddingModelProviders.KIMI]: {
        modelName,
        apiKey: await getDecryptedKey(customModel.apiKey || settings.kimiApiKey),
        timeout: 10000,
        batchSize: getSettings().embeddingBatchSize,
        configuration: {
          baseURL: customModel.baseUrl || ProviderInfo[EmbeddingModelProviders.KIMI].host,
          fetch: customModel.enableCors ? safeFetch : undefined,
        },
      },
      [EmbeddingModelProviders.DEEPSEEK]: {
        ...
        batchSize: getSettings().embeddingBatchSize,
        configuration: {
          baseURL: customModel.baseUrl || ProviderInfo[EmbeddingModelProviders.DEEPSEEK].host,
          fetch: customModel.enableCors ? safeFetch : undefined,
        },
      },
