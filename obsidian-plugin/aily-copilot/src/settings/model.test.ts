...
describe("sanitizeSettings - Aily mode migration", () => {
  it("migrates hidden legacy default modes to Chat", () => {
    const oldSettings = {
      ...DEFAULT_SETTINGS,
      defaultChainType: ChainType.AILY_CHAIN,
    } as CopilotSettings;

    const sanitized = sanitizeSettings(oldSettings);

    expect(sanitized.defaultChainType).toBe(ChainType.COPILOT_PLUS_CHAIN);
  });

  it("preserves visible DIKIWI and Projects modes", () => {
    const dikiwiSettings = {
      ...DEFAULT_SETTINGS,
      defaultChainType: ChainType.VAULT_QA_CHAIN,
    } as CopilotSettings;
    const projectSettings = {
      ...DEFAULT_SETTINGS,
      defaultChainType: ChainType.PROJECT_CHAIN,
    } as CopilotSettings;

    expect(sanitizeSettings(dikiwiSettings).defaultChainType).toBe(ChainType.VAULT_QA_CHAIN);
    expect(sanitizeSettings(projectSettings).defaultChainType).toBe(ChainType.PROJECT_CHAIN);
  });
});
