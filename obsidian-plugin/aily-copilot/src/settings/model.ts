...
export interface CopilotSettings {
  ...
  /** Aily backend URL used by Chat, DIKIWI, and Projects. */
  ailyApiBaseUrl: string;
  /** Optional bearer token for an authenticated Aily backend. */
  ailyApiToken: string;
  /** Route Aily chat through the backend's configured external LLM providers. */
  ailyUseLlm: boolean;
  /** Automatically submit new vault files to Aily's DIKIWI foundation ingestion. */
  dikiwiAutoTrigger: boolean;
  /** Comma-separated file extensions eligible for automatic DIKIWI ingestion. */
  dikiwiAutoTriggerFileExtensions: string;
  /** Optional comma-separated path prefixes; empty means the whole vault. */
  dikiwiAutoTriggerInclusions: string;
  /** Comma-separated path prefixes excluded from automatic DIKIWI ingestion. */
  dikiwiAutoTriggerExclusions: string;
...
function resolveDefaultModelKey(settings: CopilotSettings): string {
  const activeModelKeys = new Set(
    (settings.activeModels || [])
      ...
      .map((model) => getModelKeyFromModel(model))
  );

  if (settings.defaultModelKey && activeModelKeys.has(settings.defaultModelKey)) {
...
export function sanitizeSettings(settings: CopilotSettings): CopilotSettings {
  ...
  const visibleDefaultChainTypes = new Set<ChainType>([
    ChainType.COPILOT_PLUS_CHAIN,
    ChainType.VAULT_QA_CHAIN,
    ChainType.PROJECT_CHAIN,
  ]);
  if (!visibleDefaultChainTypes.has(settingsToSanitize.defaultChainType)) {
    settingsToSanitize.defaultChainType = DEFAULT_SETTINGS.defaultChainType;
  }

  if (!settingsToSanitize.defaultModelKey) {
    settingsToSanitize.defaultModelKey = DEFAULT_SETTINGS.defaultModelKey;
  }

  if (!Array.isArray(settingsToSanitize.activeModels)) {
    settingsToSanitize.activeModels = BUILTIN_CHAT_MODELS.map((model) => ({
      ...model,
      enabled: true,
    }));
  }
  ...
  const ailyApiBaseUrl = (settingsToSanitize.ailyApiBaseUrl || "").trim();
  sanitizedSettings.ailyApiBaseUrl =
    ailyApiBaseUrl.length > 0 ? ailyApiBaseUrl : DEFAULT_SETTINGS.ailyApiBaseUrl;

  if (typeof sanitizedSettings.ailyApiToken !== "string") {
    sanitizedSettings.ailyApiToken = DEFAULT_SETTINGS.ailyApiToken;
  }

  if (typeof sanitizedSettings.ailyUseLlm !== "boolean") {
    sanitizedSettings.ailyUseLlm = DEFAULT_SETTINGS.ailyUseLlm;
  }

  if (typeof sanitizedSettings.dikiwiAutoTrigger !== "boolean") {
    sanitizedSettings.dikiwiAutoTrigger = DEFAULT_SETTINGS.dikiwiAutoTrigger;
  }
  if (typeof sanitizedSettings.dikiwiAutoTriggerFileExtensions !== "string") {
    sanitizedSettings.dikiwiAutoTriggerFileExtensions =
      DEFAULT_SETTINGS.dikiwiAutoTriggerFileExtensions;
  }
  if (typeof sanitizedSettings.dikiwiAutoTriggerInclusions !== "string") {
    sanitizedSettings.dikiwiAutoTriggerInclusions = DEFAULT_SETTINGS.dikiwiAutoTriggerInclusions;
  }
  if (typeof sanitizedSettings.dikiwiAutoTriggerExclusions !== "string") {
    sanitizedSettings.dikiwiAutoTriggerExclusions = DEFAULT_SETTINGS.dikiwiAutoTriggerExclusions;
  }
...
function mergeAllActiveModelsWithCoreModels(settings: CopilotSettings): CopilotSettings {
  settings.activeModels = filterUnsupportedChatModels(
    mergeActiveModels(settings.activeModels, BUILTIN_CHAT_MODELS)
  );
...
function filterUnsupportedEmbeddingModels(models: CustomModel[]): CustomModel[] {
  const supportedProviders = new Set([
    EmbeddingModelProviders.DEEPSEEK,
    EmbeddingModelProviders.KIMI,
  ]);
...
function filterUnsupportedChatModels(models: CustomModel[]): CustomModel[] {
  const supportedProviders = new Set([ChatModelProviders.DEEPSEEK, ChatModelProviders.KIMI]);
  return models.filter((model) => supportedProviders.has(model.provider as ChatModelProviders));
}
