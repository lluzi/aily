...
export const QASettings: React.FC = () => {
  ...
  return (
    <div className="tw-space-y-4">
      <section>
        <div className="tw-mb-3 tw-text-xl tw-font-bold">DIKIWI Foundation</div>
        <div className="tw-space-y-4">
          <SettingItem
            type="switch"
            title="DIKIWI Automatic Triggering"
            description="When enabled, newly created eligible vault files are submitted to the configured Aily backend for automatic Data -> Information -> Knowledge ingestion."
            checked={settings.dikiwiAutoTrigger}
            onCheckedChange={(checked) => updateSetting("dikiwiAutoTrigger", checked)}
          />

          <SettingItem
            type="text"
            title="Auto-Trigger File Types"
            description="Comma-separated file extensions that can be submitted to DIKIWI when created in the vault."
            value={settings.dikiwiAutoTriggerFileExtensions}
            onChange={(value) => updateSetting("dikiwiAutoTriggerFileExtensions", value)}
            placeholder="md,pdf,docx,pptx,txt"
          />

          <SettingItem
            type="textarea"
            title="Auto-Trigger Inclusions"
            description="Optional comma- or line-separated path prefixes to watch. Leave empty to allow the whole vault, subject to exclusions."
            value={settings.dikiwiAutoTriggerInclusions}
            onChange={(value) => updateSetting("dikiwiAutoTriggerInclusions", value)}
            placeholder="00-Chaos"
            rows={3}
          />

          <SettingItem
            type="textarea"
            title="Auto-Trigger Exclusions"
            description="Comma- or line-separated path prefixes excluded from automatic DIKIWI ingestion. Generated DIKIWI output folders are excluded by default to prevent processing loops."
            value={settings.dikiwiAutoTriggerExclusions}
            onChange={(value) => updateSetting("dikiwiAutoTriggerExclusions", value)}
            rows={4}
          />
                    ...
                    content={
                      <div className="tw-flex tw-max-w-96 tw-flex-col tw-gap-2">
                        <div className="tw-pt-2 tw-text-sm tw-text-muted">
                          ...
                          semantic search and DIKIWI retrieval functionality. Changing the
                        ...
                        <ul className="tw-pl-4 tw-text-sm tw-text-muted">
                          ...
                          <li>Impact DIKIWI retrieval performance</li>
                        </ul>
                      </div>
                    }
                  />
                        ...
                        <ul className="tw-list-disc tw-space-y-1 tw-pl-2 tw-text-sm">
                          ...
                          <li>
                            <div className="tw-flex tw-items-center tw-gap-1">
                              ...
                              <span>Updates when entering DIKIWI mode (Recommended)</span>
                            </div>
                          </li>
                        </ul>
                      </div>
          ...
          <SettingItem
            ...
            description="When enabled, Copilot index won't be loaded on mobile devices to save resources. Only chat mode will be available. Any existing index from desktop sync will be preserved. Uncheck to enable DIKIWI mode on mobile."
