...
const ChatToolControls: React.FC<ChatToolControlsProps> = ({
  ...
  // If not Chat/Projects, don't show any tools
  if (!isCopilotPlus) {
    return null;
  }
  ...
  return (
    <TooltipProvider delayDuration={0}>
      ...
      <div className="tw-hidden tw-items-center tw-gap-1.5 @[420px]/chat-input:tw-flex">
        {/* Autonomous Agent button - only show in Chat mode and NOT in Projects mode */}
        {showAutonomousAgent && (
          <Tooltip>
      ...
      <div className="tw-flex tw-items-center tw-gap-0.5 @[420px]/chat-input:tw-hidden">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="tw-w-56">
            {/* Autonomous Agent option - only show in Chat mode and NOT in Projects mode */}
            {showAutonomousAgent && (
              <DropdownMenuItem
                onClick={handleAutonomousAgentToggle}
